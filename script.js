...
function initWorkflowAnimation() {
  ...
  const handleScroll = () => {
    ...
    if (rect.bottom > 0 && rect.top < windowHeight) {
      ...
      const scrollProgress = Math.max(
        0,
        Math.min(1, (windowCenter - rect.top) / sectionHeight)
      );
  ...
  function animateExtractionStep() {
    ...
    const processingDots = document.querySelectorAll(
      "#step2 .processing-dots span"
    );
  ...
  function animateResultsStep() {
    ...
    const summaryStats = document.querySelectorAll(
      "#step5 .summary-stats .stat-item"
    );
...
function initScrollAnimations() {
  ...
  const animatedElements = document.querySelectorAll(
    ".pain-card, .benefit-card, .step, .customer-content"
  );
